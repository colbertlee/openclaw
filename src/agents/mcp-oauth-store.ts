...
export { parseMcpOAuthStoreJson, type McpOAuthStore } from "./mcp-oauth-store.kernel.js";
...
export async function readMcpOAuthStoreReadOnly(
  ...
  return (
    (await runOpenClawStateWorkerOperation(
      context,
      (scope) => scope.execute({ type: "mcpOAuth.readOnly", input: storeKey }),
      { existingOnly: true },
    )) ?? {}
  );
}
...
export async function listMcpOAuthStoreKeysByPrefix(
  ...
  return (
    (await runOpenClawStateWorkerOperation(
      context,
      (scope) => scope.execute({ type: "mcpOAuth.keys", input: prefix }),
      { existingOnly: true },
    )) ?? []
  );
}
...
export async function countMcpOAuthStorePrincipals(prefix: string): Promise<number> {
  return (
    (await runOpenClawStateWorkerOperation(
      ...
      (scope) => scope.execute({ type: "mcpOAuth.countPrincipals", input: prefix }),
      { existingOnly: true },
    )) ?? 0
  );
}
...
export async function readMcpOAuthPendingAuthorization(
  ...
  return runOpenClawStateWorkerOperation(
    context,
    (scope) => scope.execute({ type: "mcpOAuth.pending", input: state }),
    { existingOnly: true },
  );
}
